...
export function processColorArgs(color: Color, gradient: Gradient) {
    ...
    if (gradient) {
        colorClassNames.push(getColorGradient(
            gradient.cStart,
            gradient.csStart,
            gradient.cEnd,
            gradient.csEnd,
...
export function getColorGradient(cStart: string, csStart: string, cEnd: string, csEnd: string, gradientType: string): string {
    if (!['background', 'fill', 'color', 'text'].includes(gradientType)) {
        gradientType = "background";
    }
    return ["color", gradientType, "gradient", cStart, csStart, "to", cEnd, csEnd].join("-");
}
